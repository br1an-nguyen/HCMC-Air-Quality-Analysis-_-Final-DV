...
.dash-header h1 {{ color:#fff; font-size:21px; font-weight:700; margin:0 0 6px; letter-spacing:-.4px; }}
.dash-header .hm {{ color:rgba(255,255,255,.55); font-size:12.5px; margin:0; display:flex; gap:16px; flex-wrap:wrap; }}
.kpi-card {{
...
.kpi-card .klabel {{ font-size:10px; font-weight:600; letter-spacing:.9px; color:{C['sub']}; text-transform:uppercase; margin-bottom:8px; display:flex; align-items:center; gap:6px; }}
.kpi-card .kdot {{ width:7px; height:7px; border-radius:50%; flex-shrink:0; }}
.kpi-card .kval {{ font-size:30px; font-weight:700; line-height:1; margin-bottom:4px; letter-spacing:-1px; font-family:'DM Mono',monospace; }}
.kpi-card .kunit {{ font-size:12px; font-weight:400; color:{C['sub']}; margin-left:2px; }}
.kpi-card .ksub {{ font-size:11px; color:{C['sub']}; }}
.stitle {{ font-size:14px; font-weight:700; color:{C['text']}; margin:0 0 2px; letter-spacing:-.2px; }}
...
.rtable th {{ padding:6px 10px; text-align:left; font-size:9.5px; font-weight:700; color:{C['sub']}; letter-spacing:.6px; text-transform:uppercase; }}
...
""", unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        st.markdown("**Dữ liệu**")
        uploaded = st.file_uploader("Tải CSV", type="csv", help="Air_Quality_HCMC_Cleaned.csv", key="goal4_csv_upload")
        st.markdown("---")
        st.markdown("**Bộ lọc**")
        station_ph = st.empty()
        date_ph    = st.empty()
    ...
    if uploaded:
        ...
        try:
            df_raw = load_data(str(Path(__file__).parent.parent / "data" / "cleaned" / "Air_Quality_HCMC_Cleaned.csv"))
  ...
  <div class="hm">
    ...
    with r1_right:
        st.markdown('<p class="stitle">Hệ số Pearson r — 6 cặp biến</p>'
                    '<p class="ssub">Mức độ tương quan tuyến tính · toàn bộ trạm</p>',
                    unsafe_allow_html=True)
        rows_html = "".join(
...
<table class="rtable">
  ...
  <tbody>{rows_html}</tbody>
    ...
    # ── Row 2: Pair-scatter CO trung tâm
    ...
    for col_ui, (xa, ya), lbl in zip(st.columns(3, gap="medium"), pairs3, labels3):
        ...
        fig_p.update_layout(
            ...
            yaxis=dict(title=dict(text=ya,font=dict(size=9.5)),
                       gridcolor=C["grid"], zeroline=False, tickfont=dict(size=8.5)),
            legend=dict(title=dict(text="Trạm",font=dict(size=8.5)), font=dict(size=8.5),
                ...
                if color_by == "Station_No":
                    ...
                    fig_sc.update_layout(
                        **bl(340), xaxis=_axis_x, yaxis=_axis_y, hovermode="closest",
                        legend=dict(title=dict(text="Trạm",font=dict(size=11)),
                                    bgcolor="rgba(255,255,255,.85)", bordercolor=C["border"],
                    ...
                    fig_sc.update_layout(
                        **bl(340), xaxis=_axis_x, yaxis=_axis_y, hovermode="closest",
                        ...
                        coloraxis_colorbar=dict(
                            title=dict(text="Giờ",font=dict(size=11)), thickness=14, len=.75,
                            tickfont=dict(size=10), tickvals=[0,6,12,18,23],
                            ticktext=["0h","6h","12h","18h","23h"], x=1.01),
                    )
